...
describe('console output', function() {
  const warning = 'WARNING - content-type is json but body type is not set\n'
  ...
  it('should warn developers if there is a header with \'json\' but the body type is not JSON', function() {
    ...
    unhook()
    chai.assert.equal(warning, stripAnsi(stdout), 'expect stdout to have a specific warning')
  })
