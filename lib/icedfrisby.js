...
class Frisby {
   ...
   */
  expectOneHeader (header, content) {
    const self = this

    header = (header + "").toLowerCase()

    this.current.expects.push(function () {
      if(self.current.response.headers[header] instanceof Array){
        throw new Error("Header '" + header + "' present more than once in HTTP response")
      }
      if (typeof self.current.response.headers[header] !== "undefined") {
        if(content instanceof RegExp){
          chai.expect(self.current.response.headers[header]).to
            .match(content)
        }
   ...
   * @desc HTTP header expect helper
   */
  expectAnyHeader (header, content) {
    const self = this
    ...
    this.current.expects.push(function () {
      if (typeof self.current.response.headers[header] !== "undefined") {
        var responseHeaders = [].concat(self.current.response.headers[header])
        if(content instanceof RegExp){
          ...
          chai.expect(responseHeaders).to
            .include.something.that.satisfies(thisHeader => thisHeader.toLowerCase().equals(content.toLowerCase()))
   ...
   * @param {string} content - header value content
   * @return {object}
   * @desc Alias for expectOneHeader
   */
  expectHeader (header, content) {
    return this.expectOneHeader(header,content)
  }
   ...
   */
  expectOneHeaderContains (header, content) {
    const self = this
    header = (header + "").toLowerCase()
    this.current.expects.push(function () {
      if(self.current.response.headers[header] instanceof Array){
        throw new Error("Header '" + header +
        "' present more than once in HTTP response")
      }
      if (typeof self.current.response.headers[header] !== "undefined") {
        chai.expect(self.current.response.headers[header].toLowerCase()).to
   ...
   * @desc HTTP header expect helper (using 'contains' instead of 'equals')
   */
  expectAnyHeaderContains (header, content) {
    const self = this
    header = (header + "").toLowerCase()
    this.current.expects.push(function () {
      var responseHeaders = [].concat(self.current.response.headers[header])
   ...
   * @param {string} content - header value content
   * @return {object}
   * @desc Alias for expectOneHeaderContains
   */
  expectHeaderContains (header, content) {
    return this.expectOneHeaderContains(header,content)
  }
  ...
  expectHeaderToMatch (header, pattern) {
    return this.expectOneHeader(header,pattern)
  }
