...
class Frisby {
  ...
  expectHeader (header, content, options) {
    ...
    options = {
      allowMultipleHeaders: ((options && options.hasOwnProperty('allowMultipleHeaders')) ? options.allowMultipleHeaders : false)
    }
  ...
  expectHeaderContains (header, content, options) {
    ...
    this.current.expects.push(function () {
      ...
      if (typeof self.current.response.headers[header] !== "undefined") {
        var responseHeaders = [].concat(self.current.response.headers[header]).map(thisHeader => thisHeader.toLowerCase())
        chai.expect(responseHeaders).to
