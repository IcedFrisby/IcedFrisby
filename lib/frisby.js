...
//
// Mocha-Friendly Frisby is a library designed to easily test REST API endpoints and their responses with node.js and Mocha
//
  ...
  this.current.expects.push(function() {
    ...
    if("*" === type || "&" === type) {
      _.forEach(jsonBody, function(json) {
        ...
        expect(json).to.deep.equal(jsonTest); //TODO self.current.isNot case
      });
    ...
    // ONE item in array should match
      ...
      jsonBody.should.include.something.that.deep.equals(jsonTest); // TODO self.current.isNot case

    // Normal matcher
    } else {
      expect(jsonBody).to.deep.equal(jsonTest); //TODO self.current.isNot case
    }
  });
  return this;
};
