...
function applyPath(path, jsonBody, isNot) {
  ...
  try {
    ...
    if (!isNot) {
      ...
      console.warn(
        "[IcedFrisby] You attempted to traverse through an object with a path ('" +
          path +
          "') that did not exist in the object. \
...
pathMatch.matchJSON = function(check) {
  ...
  if ('*' === lastOption) {
    ...
    checkTypes.assert.array(
      check.jsonBody,
      "Expected an Array in the path '" +
        check.path +
        "' but got " +
        typeof check.jsonBody
    )
    ...
    if (check.isNot) {
      expect(check.jsonBody).to.not.include.something.that.deep.equals(
        check.jsonTest
      )
    } else {
      expect(check.jsonBody).to.include.something.that.deep.equals(
        check.jsonTest
      )
    }
...
pathMatch.matchContainsJSON = function(check) {
  ...
  if (
    checkTypes.not.object(check.jsonBody) &&
    checkTypes.not.array(check.jsonBody)
  ) {
    throw new Error(
      'ContainsJSON does not support non-Array/Object datatypes. Got ' +
        typeof check.jsonBody +
        ' in JSON body'
    )
  } else if (
    checkTypes.not.object(check.jsonTest) &&
    checkTypes.not.array(check.jsonTest)
  ) {
    throw new Error(
      'ContainsJSON does not support non-Array/Object datatypes. Got ' +
        typeof check.jsonBody +
...
pathMatch.matchJSONTypes = function(check) {
  ...
  if ('*' === lastOption) {
    ...
    if (check.isNot) {
      ...
      if (0 !== delta) {
        throw new Error(
          'Expected all objects to be invalid but ' +
            delta +
            '/' +
            check.jsonBody.length +
            ' objects validated successfully'
        )
      }
    }
...
pathMatch.matchJSONLength = function(check) {
  ...
  checkTypes.assert.integer(
    check.jsonTest.count,
  ...
  if ('*' === lastOption) {
    ...
    if (check.isNot) {
      ...
      if (0 !== delta) {
        throw new Error(
          'Expected all lengths to be invalid but ' +
            delta +
            '/' +
            check.jsonBody.length +
            ' lengths validated successfully'
        )
      }
    }
