...
var icedFrisbyConfig = require('./icedfrisbyconfig');
...
var globalSetup = function(opts) {
  ...
  if (!_.isUndefined(opts)) {
    ...
    if (_frisbyGlobalAlreadySetup) {
        throw new Error('IcedFrisby global setup has already been done. Doing so again is disabled, and it is encouraged to use a configuration file');
    }

    // set the global variable _frisbyGlobalAlreadySetup to true
    _frisbyGlobalAlreadySetup = true;

    // merge the passed in options with the existing configuration
    icedFrisbyConfig._setGlobalSetup(opts);

    } else {
        return icedFrisbyConfig._getConfig();
    }
};
...
//
function Frisby(msg) {
  // Clone IcedFrisby configuration (not reference).
  var config = _.clone(icedFrisbyConfig._getConfig(), true);

  // config may contain mixed-cased header names, the code expects lowercase however.
  if(config.request && config.request.headers) {
    var _tmpHeaders = {};
    _.forEach(config.request.headers, function(val, key) {
      _tmpHeaders[(key+"").toLowerCase()] = val+"";
    });
    config.request.headers = _tmpHeaders;
  }
  ...
  this.current = {
    ...
    retry_backoff: config.retry_backoff || 1000,
...
// any baseUri setup in icedFrisbyConfig
//
...
Frisby.prototype.reset = function() {
  this.current.request = _.cloneDeep(icedFrisbyConfig._getDefaultConfig().request);
  return this;
};
