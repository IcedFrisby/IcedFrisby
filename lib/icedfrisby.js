...
class Frisby {
  ...
  retry (count, backoffMillis) {
    ...
    if (typeof backoffMillis !== "undefined") {
      this.current.retry_backoff = backoffMillis
    }
    return this
  }
  ...
  _mochaTimeout () {
    ...
    const gracePeriodMillis = 100
    return this._timeout +
      this.current.retry * (this.current.retry_backoff + this._timeout) +
      gracePeriodMillis
  }
  ...
  _makeRequest (done) {
    ...
    const handleRequestTimeout = () => {
      ...
      this._noteExpectationFailed(Error(message))
    ...
    this.current.it(() => {
      ...
      if (this.current.response.status >= 500 && maybePerformRetry()) {
        return
      }
