...
//
// Frisby is a library designed to easily test REST API endpoints and their responses with node.js and Mocha
//
  ...
  this.current.expects.push(function() {
    ...
    if("*" === type || "&" === type) {
      _.forEach(jsonBody, function(json) {
        ...
        expect(json).to.include(jsonTest); //TODO self.current.isNot
      });
    ...
    // ONE item in array should match
      ...
      for(var i = 0; i < itemCount; i++) {
        try {
          _jsonContains(jsonBody[i], jsonTest, self.current.isNot);
        } catch (e) {
          errorCount++;
        }
      };
      ...
      if(itemCount === errorCount && !self.current.isNot) {
        var objPlural = itemCount > 1 ? "objects" : "object";
        throw new Error("Expected one out of " + itemCount + " " + objPlural + " to match provided JSON");
      }

    // Normal matcher
    } else {
      expect(jsonBody).to.include(jsonTest); //TODO self.current.isNot
    }
  });
  return this;
};
  ...
  this.current.expects.push(function() {
    ...
    var expectLength = function(jsonBody, lengthSegments) {
      ...
      switch (lengthSegments.sign) {
        case "<=":
          var msg = "Expected JSON length to be less than or equal '" + lengthSegments.count + "', got '" + len + "'" + (path ? (" in path '" + path + "'") : "");
          expect(len).to.be.lessThan(lengthSegments.count + 1, msg);
          break;
        case "<":
          var msg = "Expected JSON length to be less than '" + lengthSegments.count + "', got '" + len + "'" + (path ? (" in path '" + path + "'") : "");
          expect(len).to.be.lessThan(lengthSegments.count, msg);
          break;
        case ">=":
          var msg = "Expected JSON length to be greater than or equal '" + lengthSegments.count + "', got '" + len + "'" + (path ? (" in path '" + path + "'") : "");
          expect(len).to.be.greaterThan(lengthSegments.count - 1);
          break;
        case ">":
          var msg = "Expected JSON length to be greater than '" + lengthSegments.count + "', got '" + len + "'" + (path ? (" in path '" + path + "'") : "");
          expect(len).to.be.greaterThan(lengthSegments.count);
          break;
        case null:
          var msg = "Expected JSON length to be '" + lengthSegments.count + "', got '" + len + "'" + (path ? (" in path '" + path + "'") : "");
          expect(len).to.equal(lengthSegments.count);
          break;
      } //end switch
    }
