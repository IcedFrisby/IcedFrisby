...
class Frisby {
   ...
   */
  reset () {
    throw Error('reset() has been removed from IcedFrisby v2.0+ - there\'s no more globalSetup(), use config() instead')
  }
   ...
   * @return {boolean}
   * @desc HTTP body expect length helper
   */
  expectBodyLength (length, operator) {
    return this._expect(() => {
      if (!_.isUndefined(this._response.body)) {
        switch(operator){
          case "<":
            chai.expect(this._response.body).to.have.lengthOf.below(length)
            break
          case ">":
            chai.expect(this._response.body).to.have.lengthOf.above(length)
            break
          case ">=":
            chai.expect(this._response.body).to.have.lengthOf.at.least(length)
            break
          case "<=":
            chai.expect(this._response.body).to.have.lengthOf.at.most(length)
            break
          default:
            chai.expect(this._response.body).to.have.lengthOf(length)
        }
      } else {
        throw Error(
          "No HTTP response body was present or HTTP response was empty"
      ...
      for(const request_option in request_options){
        if(request_options.hasOwnProperty(request_option)){
          if(request_option == "headers"){
            ...
            this.current.request[request_option] = request_options[request_option]
          }
        }
      }
    }
