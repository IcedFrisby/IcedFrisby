...
class Frisby {
   ...
   */
  reset () {
    throw Error('reset() has been removed from IcedFrisby v2.0+ - there\'s no more globalSetup(), use configure() instead')
  }
    ...
    if ('request' in opts) {
      for(const request_option in opts.request){
        if(opts.request.hasOwnProperty(request_option)){
          let value = opts.request[request_option]
          if(typeof opts.request[request_option] === "object"){
            value = JSON.parse(JSON.stringify(opts.request[request_option])) //break reference to previous object
          }
          this.current.request[request_option] = value
        }
      }
    }
    if ('json' in opts) {
